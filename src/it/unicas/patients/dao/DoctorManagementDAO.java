...
public class DoctorManagementDAO {
	...
	public static Doctors getDoctorById(String doctorId) {
		...
		{
			Connection conn = DBUtil.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM doctor_id WHERE id = ?");
			ps.setString(1, doctorId);
			ResultSet rs = ps.executeQuery();
			while(rs.next())
			{
				doctor = new Doctors(rs.getString("prod_id"),
						rs.getString("dr_name"),
						rs.getString("dr_phone"),
						rs.getString("dr_email"),
						rs.getString("created_date"));
	...
	public static int addDoctor(Doctors doctor) {
		...
		try
		{
			...
			ps.setString(4, doctor.getCreatedDate());
	...
	public static int deleteDoctor(String doctor_id) {
		int status = 0;
		try
		{
			Connection conn = DBUtil.getConnection();
